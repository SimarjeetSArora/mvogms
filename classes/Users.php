		...
		if($check > 0){
			...
			if($save){
				...
				if(isset($_FILES['id_proof']) && $_FILES['id_proof']['tmp_name'] != ''){
					if(!is_dir(base_app."uploads/vendors/id"))
					mkdir(base_app."uploads/vendors/id");
					$fname = 'uploads/vendors/id/id_'.($vid).'.png';
					$dir_path =base_app. $fname;
					$upload = $_FILES['id_proof']['tmp_name'];
					$type = mime_content_type($upload);
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						$resp['msg'].=" But Image failed to upload due to invalid file type.";
						...
						if($gdImg){
								...
								imagedestroy($t_image);
								if($uploaded_img){
									$qry = $this->conn->query("UPDATE `vendor_list` set id_proof = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$vid' ");
									if($this->settings->userdata('id') == $id && $this->settings->userdata('login_type') == 2)
										$this->settings->set_userdata('id_proof',$fname."?v=".(time()));
								}
						}else{
						$resp['msg'].=" But Image failed to upload due to unkown reason.";
						}
